# ADR-0001: Modern TypeScript + React stack & target architecture

- **Status:** Accepted
- **Date:** 2026-06-20
- **Deciders:** SauceTaster (maintainer)
- **Supersedes:** the Express/Mongoose/Pug-upgrade parts of `docs/MODERNIZATION.md`
- **Evidence:** synthesized from 7 current-docs-grounded research tracks; the same
  content with full per-area citations is in `docs/research/2026-06-stack-research.md`.

## Context

Vulndesk is being rebuilt (strangler-style) from the inherited Express/Mongoose/Pug
app to a modern TypeScript stack while preserving Vulnogram behavior/UX (see
`docs/REWRITE.md`). This ADR locks the concrete, best-practice package choices
within the agreed high-level stack (TypeScript · Hono · React + TanStack · Zod ·
Drizzle/Postgres · BetterAuth · Tailwind), grounded in current (2026) docs, and
resolves the auth model against the MCP authorization spec. Versions were verified
against npm on 2026-06-19; anything unverifiable in current docs is flagged ⚠️ and
carried as a risk.

## Decision

Adopt the choices below. Detailed rationale, rejected alternatives, risks, and the
implementation sequencing follow.
